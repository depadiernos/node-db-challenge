...
- [x] Explain the difference between `Relational Databases` and `SQL`.
  > `Relational Databases` are a specific type of database that an SQL database is a part of. `SQL` specifically refers to the Query Language used by `SQL` databases to pull data from the database.
- [x] Why do tables need a `primary key`?
  > This is the main identifier for a specific piec of data (row). A primary key will always refer to only 1 row.
- [x] What is the name given to a table column that references the primary key on another table.
  > A foreign key.
- [x] What do we need in order to have a _many to many_ relationship between two tables.
  > A `Join Table` where the primary keys of the two tables are foreign keys and the primary key is typically the combination of both foreign keys.
